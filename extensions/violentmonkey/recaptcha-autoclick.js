...
// @version     1.0
// @description Automatically click reCaptcha checkbox button when detected, but it doesn't solve the captcha for you!
...
(function() {
    ...
    function clickRecaptchaCheckbox() {
        let checkbox = document.querySelector('.recaptcha-checkbox-checkmark');
        if (checkbox) {
            checkbox.click();
        } else {
            console.error("Unable to find reCAPTCHA checkbox element");
        }
    }
    ...
    window.addEventListener('load', function() {
        setTimeout(init, 5000); // Stop after 5 seconds
    });

    // Set up an interval for the first 5 seconds (if the page hasn't loaded yet)
    let captchaInterval = setInterval(init, 500);
})();
